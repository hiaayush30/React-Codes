//creating a middleware for handling the pagination
...
app.get("/todos",paginatedMiddleware(todos),(req,res)=>{
   res.json({
    todos:req.resultModel
   })
})
...
function paginatedMiddleware(model){
    ...
    const resultModel={};
    resultModel.result=model.slice(startIndex,endIndex) //so todos of index 0 to 4 (ie 1 to 5)
    ...
    if(startIndex>0){
        resultModel.previous={
            page:page-1,
            limit:limit
        }
    }
    req.resultModel=resultModel;
    next()
    }
}
